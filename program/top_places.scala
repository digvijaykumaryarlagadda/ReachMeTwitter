...
val mytempvar= sqlContext.sql("SELECT user.location FROM tweets")
/*
val mytempvar= sqlContext.sql("SELECT user.location, COUNT(*) AS temptable FROM tweets GROUP BY user.location ORDER BY temptable DESC LIMIT 10").collect().foreach(println)
*/
val mytempvar2 = mytempvar.map(t => (t(0),1)).reduceByKey(_+_).sortBy(-_._2)
mytempvar2.coalesce(1,true).saveAsTextFile("topPlacesOutput")
